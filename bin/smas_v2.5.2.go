...
type Config struct {
	...
	StreamReport struct {
		...
		ReportDate    string   `json:"reportDate"` // "today" 或 "yesterday"
...
type PushPlusConfig struct {
	...
	Topic   string `json:"topic,omitempty"` // 群组编码，可选
}
...
func checkPingState(address string) bool {
	...
	if useTCP {
		...
		if !tcpOK {
			log.Printf("[ monitorPing ] %s TCP ping (port %d) failed", address, targetSshPort)
			successTcp = false
		}
	}
...
func sendAlert(alertLevel string, data EmailTemplateData) {
	...
	// 判断发送PushPlus
	if config.AlertMethods.PushPlus {
		sendPushPlusAlert(config.PushPlus, alertLevel, data)
	} else {
		// 如果PushPlus禁用，则记录日志
		log.Printf("PushPlus告警通知被禁用，仅记录日志: %s - %s", data.Subject, data.Message)
	}

	// 如果所有方式都禁用，则记录日志
	if !config.AlertMethods.Email && !config.AlertMethods.WechatWork && !config.AlertMethods.PushPlus {
		log.Printf("所有告警通知方式均被禁用，仅记录日志: %s - %s", data.Subject, data.Message)
	}
