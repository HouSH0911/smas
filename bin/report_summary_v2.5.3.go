...
const historyFileName = "alert_history.csv"
const legacyHistoryFileName = "alert_history.json"
const dataDir = "../data"
...
func legacyBackupAlertHistoryFile() {
	historyPath := getHistoryPath()
	...
	backupFileName := fmt.Sprintf("alert_history_%s_%s.csv", dateStr, reportType)
...
// 从文件加载告警历史
func legacyLoadAlertHistory() { // 首字母大写供 main 调用
	// 确保 data 目录存在
	if err := os.MkdirAll(dataDir, 0755); err != nil {
		log.Printf("创建 data 目录失败: %v", err)
		return
