...
func reloadConfig() error {
	...
	// 更新全局配置
	config = newConfig
	log.Printf("配置已重新加载 - 邮件发送: %v, 邮件接收：%v, 进程监控开关: %v, 端口监控开关: %v,Ping通信监控开关: %v,资源监控开关: %v,目录文件监控开关: %v,目标服务器端口监控开关: %v",
		config.Email.From, config.Email.Recipients, config.Monitor.ProcessMonitor, config.Monitor.PortMonitor, config.Monitor.ServerReachMonitor, config.Monitor.Resource, config.Monitor.DirFileMonitor, config.Monitor.RaPortMonitor)
	return nil
}
