...
func checkPort(address, port string) bool {
	...
	for i := 0; i < maxRetries; i++ {
		...
		time.Sleep(50 * time.Millisecond) // 每次重试间隔
	}
...
func monitorServersProcess(config Config, statuses map[string]*ServerStatus) {
	...
	for _, server := range config.Servers {
		...
		// if statuses[key] == nil {
		// 	statuses[key] = &ServerStatus{
		// 		ProcessAlertSent: make(map[string]bool),
		// 	}
		// }
...
func monitorDirectory(config Config, statuses map[string]*ServerStatus) {
	...
	for _, server := range config.Servers {
		...
		// serverState := checkPingState(address)
		// if !serverState {
		// 	fmt.Println("Server has lost connection, do not detect directory.")
		// 	continue
		// }
		resp, err := http.Get(url)
...
func sendEmail(emailConfig EmailConfig, subject, body string) {
	...
	quotedBody := quotedprintable.NewWriter(&bodyBuffer)
	quotedBody.Write([]byte(body))
	quotedBody.Close()

	//auth := smtp.PlainAuth("", from, emailConfig.Password, smtpHost)
	// err := smtp.SendMail(smtpHost+":"+smtpPort, nil, from, emailConfig.Recipients, bodyBuffer.Bytes())
	// if err != nil {
	// 	log.Fatal(err)
	// }
	// 创建TLS配置，跳过证书验证
