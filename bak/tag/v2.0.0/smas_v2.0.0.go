...
const (
	...
	checkCacheTTL     = 5   // 检查结果缓存时间(秒)
	resourceCheckFreq = 180 // 资源监控频率(秒)
	dirCheckFreq      = 300 // 目录监控频率(秒)
...
func main() {
	...
	defer ticker.Stop()
	for range ticker.C {
		log.Println("===== Unified monitoring START =====")
		unifiedMonitor(config, statuses)
		log.Println("===== Unified monitoring END =====")
	}

	// 启动监控目录任务（每个 5 分钟点的第 2 分钟执行）
