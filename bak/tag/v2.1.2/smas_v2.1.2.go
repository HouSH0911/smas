...
type Config struct {
	...
	HttpTimeout          int           `json:"httpTimeout"`          // HTTP请求超时计数
	...
	PortTimeout          int           `json:"portTimeout"`          // 端口检测超时时间（秒）
}
...
func checkPort(address, port string) bool {
	...
	// 冷却短路：如果上次失败时间距今小于冷却期，则跳过实际检测
	if v, ok := portFailures.Load(addrKey); ok {
		if t, ok2 := v.(time.Time); ok2 {
			if time.Since(t) < time.Duration(config.PortCoolDown)*time.Second {
				if time.Since(t) < 5*time.Second {
					log.Printf("[ monitorPorts ] %s in cooldown, skipping TCP/UDP check", addrKey)
				}
				return false
			}
			// 冷却已过，继续检测
	...
	// TCP 重试（保留原有重试逻辑，但确保循环边界正确）
	const tcpMaxRetries = 3
	const retryDelay = 150 * time.Millisecond
	const tcpTimeout = 3000 * time.Millisecond
	tcpOK := false
	for attempt := 1; attempt < tcpMaxRetries; attempt++ {
		tcpConn, tcpErr := net.DialTimeout("tcp", addrKey, tcpTimeout)
		if tcpErr == nil {
			tcpConn.Close()
			tcpOK = true
			break
	...
	if err != nil {
		// 解析失败，记录冷却并返回 false
		now := time.Now()
		log.Printf("[ monitorPorts ] %s UDP resolve failed, entering cooldown %v: %v", addrKey, time.Duration(config.PortCoolDown)*time.Second, err)
	...
	if udpOK {
		if _, ok := portFailures.Load(addrKey); ok {
			log.Printf("[ monitorPorts ] %s recovered (UDP), leaving cooldown", addrKey)
			portFailures.Delete(addrKey)
		}
		return true
	}

	// 如果 TCP 和 UDP 都失败，记录冷却并返回 false
	now := time.Now()
	// 只在首次进入冷却时打印一次详细日志
	if _, ok := portFailures.Load(addrKey); !ok {
		log.Printf("[ monitorPorts ] %s TCP and UDP both failed, entering cooldown for %v", addrKey, time.Duration(config.PortCoolDown)*time.Second)
	}
	portFailures.Store(addrKey, now)
