		...
		if !tracker.FirstFailureTime.IsZero() {
			...
			if tracker.AlertSent {
				...
				data := EmailTemplateData{
					Subject:   "🔄服务器重启告警",
					Server:    address,
					Message:   fmt.Sprintf("服务器监测到重启 (通信中断时长约 %s)，通信已自动恢复。", timeSinceDown.Round(time.Second)),
					Action:    "检测到服务器重启，请检查业务日志。",
					Timestamp: now.Format("2006-01-02 15:04:05"),
				}
				sendEmail(config.Email, "severe", data) // 重启通常比较重要，建议 severe
